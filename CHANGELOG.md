...
- **Control characters in an attacker-chosen `detail` could spoof the report or
  poison a generated config.** A `detail` — a path, host, or command line a
  dependency picks — is interpolated into the terminal report, the drafted
  config's comments and quoted strings, and the JSONL sink. Left raw, a
  dependency could embed **ANSI escapes** to spoof or erase findings on the TTY,
  or a **newline** to smuggle a second `default:` bucket (an allow-all self-grant)
  or a `SyntaxError` into a config generated by `dephawk init`/`--draft`. Every
  event's `detail` and `reason` now pass through control-character stripping (C0
  - DEL → space) at `createEvent`, the single funnel every reporter, sink, and
    the config renderer draw from. Stripping runs **before** redaction, so a control
    character can no longer split a token to slip a secret past `redactSecrets`
    either. (The HTML report was already HTML-escaped; the JSONL sink already
    `JSON.stringify`s each event.)
